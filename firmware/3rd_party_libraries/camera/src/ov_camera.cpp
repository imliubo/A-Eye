...
void Camera::init() {
  ...
  dvp_set_ai_addr((long)KPU::g_ai_buf, (long)(KPU::g_ai_buf + 320 * 240),
                  (long)(KPU::g_ai_buf + 320 * 320 * 2));
...
void Camera::save_picture() {
  count++;
  sprintf(buffer, "0:A-Eye-%d.bmp", count);
  rgb565tobmp((char *)(get_frame_ptr()), 320, 240, _T(buffer));
