...
class pmd {
 public:
  ...
  uint32_t *matrixnolist;  //[0:7] bone number0, [8:15] bone number 1, [16:23]
                           // ratio(0-100, 0...bone number 0(100%), 100...bone
                           // number 1(100%), 50...half half.

  uint32_t facecount;
  uint16_t *facelist;  // polygonlist[polycount*3]<- it's always triangle only!
                       // (mmd restriction)
